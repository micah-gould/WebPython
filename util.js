...
function createButton (clazz, label, action) {
  ...
  for (const cl of clazz.split(' ')) { button.classList.add(cl) }
...
function append (parent, type, content) {
  ...
  if (typeof content === 'string') { child.textContent = content } else if (content !== undefined) { child.appendChild(content) }
  return child
}
