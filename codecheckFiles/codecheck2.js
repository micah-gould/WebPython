/* eslint no-unused-vars: off, no-undef: off, camelcase: off, eqeqeq: off
    -------------
...
window.horstmann_config = {
  ...
  getInteractiveId: function (e) {
    while (e && e.parentNode && (!e.parentNode.className || e.parentNode.className.indexOf('interactivities') < 0)) { e = e.parentNode }
    return e ? e.id : null
  },
  ...
  retrieve_state: async (element, callback) => {
    if ('lti' in horstmann_config) {
      try {
        const response = await postData(horstmann_config.lti.retrieveURL, {
          submissionID: horstmann_config.lti.submissionID
        })
