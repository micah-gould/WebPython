...
window.addEventListener('load', async function () {
  ...
  function createRearrange (fileName, setup) {
    ...
    function shiftTile (tileDiv, droppedTileX) {
      const leftX = left.getBoundingClientRect().left // TODO: Adjust for margin
      const indent = Math.max(0, Math.round((droppedTileX - leftX) / indentWidth - 1))
      setIndent(tileDiv, indent)
    }

    function codeOf (tile) {
      const code = codeMap.get(tile)
      return code === undefined ? tile.textContent : code
    }

    function measureIndentWidth () {
      // Measure indent width
      const tileDiv = createTile()
      ...
      if (indentWidth === 0) { setTimeout(measureIndentWidth, 2000) } else {
    ...
    function stickyBrace (tileDiv) {
      ...
      let rightBrace = null
      for (let i = 0; rightBrace === null && i < right.children.length; i++) { if (codeOf(right.children[i]) === '}') rightBrace = right.children[i] }
      ...
      while (!done && i < left.children.length) {
        const tileDivBelow = left.children[i]
    ...
    function insertDroppedTileLeft (tileDiv, droppedTileX, droppedTileY) {
      ...
      let done = false
      const fromRight = tileDiv.parentNode === right
      let i = 0
      ...
      if (!done) { left.appendChild(tileDiv) }
    ...
    function makeTile (contents, isFixed) {
      ...
      if (isFixed) {
        ...
        tileDiv.addEventListener('dragstart', function (e) {
          ...
          const bounds = tileDiv.getBoundingClientRect()
    ...
    function moveUpDown (tileDiv, dy) {
      const parent = tileDiv.parentNode
      if (parent === right) return
      if (dy === -1) {
        const sibling = tileDiv.previousSibling
        if (sibling != tileDiv.parentNode.firstChild) { parent.insertBefore(tileDiv, sibling) }
      } else {
        const sibling = tileDiv.nextSibling
        if (sibling) { parent.insertBefore(tileDiv, sibling.nextSibling) }
      }
      tileDiv.focus()
    }
    ...
    function moveLeftRight (tileDiv, dx) {
      const parent = tileDiv.parentNode
      const indent = tileDiv.indent + dx
    ...
    const mousedownListener = function (e) {
      ...
      const tileOnLeft = [...left.children].indexOf(focusedElement) >= 0
      const tileOnRight = [...right.children].indexOf(focusedElement) >= 0
      if (!tileOnLeft && !tileOnRight) return
      if (e.target === left) {
        const p = getClientXY(e)
        const droppedTileX = p.x - indentWidth / 2
        const droppedTileY = p.y - indentWidth / 2

        insertDroppedTileLeft(focusedElement, droppedTileX, droppedTileY)
      } else if (tileOnLeft) { insertDroppedTileRight(focusedElement) }
    }
    ...
    function initialize () {
      ...
      left.addEventListener('drop', function (e) {
        left.classList.remove('dragover')
        if (drag === undefined) return // Some sort of foreign drop
        const tileDiv = drag.tile
        const p = getClientXY(e)
        const droppedTileX = p.x - drag.x
        const droppedTileY = p.y - drag.y
        drag = undefined
      ...
      if ('fixed' in setup) {
        for (const fixed of setup.fixed) { left.appendChild(makeTile(fixed, true)) }
      }
      if ('tiles' in setup) {
        for (const tile of setup.tiles) { right.appendChild(makeTile(tile, false)) }
      }
    ...
    function getState () {
      ...
      for (const tile of left.children) {
        if (tile.classList.contains('fixed')) {
          ...
          const state = { text: tile.textContent, indent: tile.indent }
          const code = codeMap.get(tile)
    ...
    function restoreState (state) {
      codeMap.clear()
      let i = 0
      const leftTiles = [...left.children]
      for (const tile of leftTiles) {
        if (tile.classList.contains('fixed')) {
          const group = state.left[i]
          i++
    ...
    function getText () {
      ...
      for (const tile of left.children) {
        ...
        if (tile.classList.contains('fixed') && code !== undefined) { content += code + '\n' } else {
  ...
  function setCommandEnabled (editor, name, enabled) {
    ...
    if (!enabled) {
      ...
      if (key && typeof key === 'object') { key = key[editor.commands.platform] }
  ...
  function setupAceEditor (editorDiv, editor, fileName, readonly) {
    ...
    if (readonly) {
      ...
      const editorHandleTextFieldEvents = function (e) {
    ...
    const tas = editorDiv.getElementsByTagName('textarea')
  ...
  function createAceEditors (fileName, setup) {
    ...
    function initialize () {
      ...
      const update = function () {
        ...
        for (const editorDiv of editorsDiv.children) {
          ...
          const lines = editorSession.getDocument().getLength()
          editor.setOptions({
            minLines: lines,
            maxLines: lines
          })
      ...
      for (let i = 0; i < setup.length; i++) {
        const contentSegment = setup[i]
        if (contentSegment === null) { continue }

        const readonly = i % 2 !== 0
        const editorDiv = document.createElement('div')
        editorDiv.classList.add('editor')
        if (fileName === 'Input') { editorDiv.classList.add('input') }
        editorDiv.textContent = contentSegment.replace(/\r?\n$/, '')
        const editor = ace.edit(editorDiv)
        if (readonly) { editorDiv.setAttribute('readonly', 'readonly') } else { editor.on('change', update) }
    ...
    function clearErrorAnnotations () {
      for (const editorDiv of editorsDiv.children) { ace.edit(editorDiv).getSession().clearAnnotations() }
    }
    ...
    function errorAnnotation (line, message) {
      ...
      for (const editorDiv of editorsDiv.children) {
        ...
        const length = editorSession.getDocument().getLength()
        totalLines += length
        if (totalLines >= line) {
          const annotations = editorSession.getAnnotations()
          annotations.push({
  ...
  function initElement (element, setup, prefix) {
    ...
    function restoreState (dummy, state) { // TODO: Eliminate dummy
      ...
      if ('studentWork' in state) { // TODO: Legacy state
        ...
        for (const entry of state.studentWork) {
          ...
          const i = fragmentName.lastIndexOf('-')
          ...
          if (editableIndex !== undefined) { work[fileName][editableIndex] = entry.code }
        }
      }
      for (const fileName in work) { editors.get(fileName).restoreState(work[fileName]) }
    ...
    function editorFor (fileName, fileSetup) {
      if ('tiles' in fileSetup) { return createRearrange(fileName, fileSetup) } else if ('editors' in fileSetup) { return createAceEditors(fileName, fileSetup.editors) } else { return createAceEditors(fileName, fileSetup) }
    }
    ...
    function appendRequiredFile (fileName, directoryPrefix) {
      ...
      const fileSetup = setup.requiredFiles[fileName]
      const editor = editorFor(fileName, fileSetup)
    ...
    function initUI () {
      ...
      const orderedFileNames = setup.order ? setup.order.split(/\s*,\s*/) : []
      const requiredFileNames = []
      const useFileNames = []
      for (let i = 0; i < orderedFileNames.length; i++) {
        const fileName = orderedFileNames[i]
        if (setup.requiredFiles[fileName]) { requiredFileNames.push(fileName) } else if (setup.useFiles[fileName]) { useFileNames.push(fileName) }
      }

      // TODO: Iterate by sort order?
      for (const fileName in setup.requiredFiles) {
        if (fileName === 'Input') {
          submitButtonLabel = _('Run')
          // Don't provide an edit box if no input required
          if (setup.requiredFiles.Input[0].trim().length > 0) { inputPresent = true } else {
      ...
      // TODO: Iterate by sort order?
      for (const fileName in setup.useFiles) { if (useFileNames.indexOf(fileName) < 0) useFileNames.push(fileName) }

      for (let i = 0; i < requiredFileNames.length; i++) { appendRequiredFile(requiredFileNames[i], directoryPrefix) }
      if (inputPresent) appendRequiredFile('Input', '')

      for (let i = 0; i < useFileNames.length; i++) {
        const fileName = useFileNames[i]

        const editorDiv = document.createElement('div')
        editorDiv.classList.add('editor')
        const text = setup.useFiles[fileName].replace(/\r?\n$/, '')
      ...
      submitButton = createButton('hc-start', submitButtonLabel, async function () {
        ...
        for (const input of form.getElementsByTagName('input')) {
          ...
          if (name !== null) { params[name] = input.getAttribute('value') }
        }
        ...
        try {
          const result = Object.keys(setup.requiredFiles)[0].split('.')[1] !== 'py' ? await postData(setup.url, params) : await python(setup, params)
      ...
      const resetButton = createButton('hc-start', _('Reset'), function () {
    ...
    function getState () {
      const work = {}
      for (const [filename, editor] of editors) { work[filename] = editor.getState() }
      return { work }
    }
    ...
    function setState (scoreText) {
      ...
      const score = element.maxscore === 0 ? 0 : element.correct / element.maxscore
    ...
    function successfulSubmission (data) {
      ...
      const start = report.indexOf('<body>')
      const end = report.indexOf('</body>')
  ...
  const elements = document.getElementsByClassName('horstmann_codecheck')
